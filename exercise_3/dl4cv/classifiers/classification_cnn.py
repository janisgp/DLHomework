...
class ClassificationCNN(nn.Module):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, kernel_size=7,
        ...
        self.pool = pool
        self.s_pool = stride_pool
        
        C, H, W = input_dim[0], input_dim[1], input_dim[2]
        H2, W2 = int(( H  - pool ) / stride_pool + 1), int(( W - pool ) / stride_pool + 1)
        
        self.conv = nn.Conv2d(C,num_filters,kernel_size,stride=stride_conv,padding=int((kernel_size-1)/2))
        
        self.conv.weight.data.mul_(weight_scale)
        
        linear_dim_1 = num_filters*H2*W2
        self.fc1 = nn.Linear(linear_dim_1,hidden_dim,bias=True)
    ...
    def forward(self, x):
        ...
        x = F.max_pool2d(x,(self.pool,self.pool),stride=self.s_pool)
